...
public class BeanFactory {
    ...
    public void init() throws IOException {
        if (scanner != null) {
            ...
            Map<String, String> autoGeneratedFixtureClasses = newHashMap();
            for (Resource resource : resources) {
                registerAutoGeneratedFixtures(autoGeneratedFixtureClasses, resource);
                registerAll(reader.read(resource));
            }

            verifyAutoGeneratedClasses(autoGeneratedFixtureClasses);
        }
    }

    private void registerAutoGeneratedFixtures(Map<String, String> autoGeneratedFixtureClasses,
                                               Resource resource) throws IOException {
        JsonNode node = objectMapper.readTree(resource.getInputStream());
        JsonNode snapshotClass = node.get(SnapshotGenerator.AUTO_GENERATED_FOR);
        if (snapshotClass != null) {
            autoGeneratedFixtureClasses.put(resource.getURI().getPath(), snapshotClass.textValue());
        }
    }
    ...
    private void verifyAutoGeneratedClasses(Map<String, String> autoGeneratedFixtureClasses) {
        autoGeneratedFixtureClasses.forEach((resource, className) -> {
            try {
                Class.forName(className);
            } catch (ClassNotFoundException e) {
                throw new RuntimeException(className + " does not exist anymore in '" + resource +
                                           "' file. If the file has been renamed/removed, please update the value of " +
                                           SnapshotGenerator.AUTO_GENERATED_FOR + " property");
            }
        });
    }
